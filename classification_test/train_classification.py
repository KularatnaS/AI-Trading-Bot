...
train_ds = CandleDataset(train_data_dir, seq_len, prediction_units)
...
model = ClassificationModel(n_trade_decisions=n_trade_decisions, seq_len=seq_len, d_model=d_model, N=N, d_ff=d_ff, dropout=dropout).to(device)
...
def run_validation(epoch):
    ...
    with torch.no_grad():
        ...
        for batch in batch_iterator_val:
            ...
            proj_output = model(encoder_input)
            label = batch['label'].to(device)
            val_loss = loss_fn(proj_output, label)
